...
import { setTimerDuration } from "@/lib/timer/timerStore";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const updatedState = await setTimerDuration(durationSeconds);
    const now = new Date();
    ...
    return NextResponse.json({
      ...
      message: "Timer duration staged successfully for later display.",
    });
  } catch (error: any) {
    return NextResponse.json(
      { success: false, message: error?.message || "Failed to set duration" },
      { status: 400 }
    );
  }
}
